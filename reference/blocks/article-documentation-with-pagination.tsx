...
type BodyBlock = Paragraph | CodeBlock;
type AdjacentArticle = { label: string; title: string };

export function ArticleDocumentationWithPagination({ ns }: { ns: string }) {
    ...
    <section className="border-t border-border bg-background py-(--section-gap)">
      <div className="mx-auto w-full max-w-3xl px-6 lg:px-8">
        <Reveal>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-4 border-b border-border pb-8 text-sm text-muted-foreground">
        ...
        <Reveal>
          <div className="mt-16 grid grid-cols-1 gap-4 border-t border-border pt-10 sm:grid-cols-2">
            <a
              href="#"
              className="group flex flex-col gap-2 rounded-md border border-border bg-card p-5 transition-colors hover:border-primary focus-visible:border-primary focus-visible:outline-none"
            >
              <span className="flex items-center gap-2 text-xs font-medium tracking-[0.2em] text-muted-foreground uppercase">
                <ArrowLeft
                  className="size-3.5 text-primary transition-transform group-hover:-translate-x-1"
                  strokeWidth={1.75}
                />
                {previous.label}
              </span>
            ...
            <a
              href="#"
              className="group flex flex-col items-end gap-2 rounded-md border border-border bg-card p-5 text-right transition-colors hover:border-primary focus-visible:border-primary focus-visible:outline-none"
            >
              <span className="flex items-center gap-2 text-xs font-medium tracking-[0.2em] text-muted-foreground uppercase">
                {next.label}
                <ArrowRight
