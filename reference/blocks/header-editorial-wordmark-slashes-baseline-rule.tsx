...
export function HeaderEditorialWordmarkSlashesBaselineRule({ ns }: { ns: string }) {
  const t = useTranslations(ns);
  const links = t.raw("links") as NavLink[];

  return (
    <header className="sticky top-0 z-50 bg-background">
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-6 px-6 py-6 lg:px-8">
        <a
          href="#inicio"
        ...
        <a
          href={t("ctaHref")}
          className="hidden text-sm font-medium tracking-tight text-primary hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary lg:inline-block"
        >
          {t("ctaLabel")}
        </a>
        ...
        <details className="group relative lg:hidden">
          ...
          <div className="fixed inset-x-0 top-[calc(4.5rem+2px)] z-50 border-b border-border bg-background px-6 py-8">
            <nav aria-label={t("navLabel")} className="flex flex-col gap-6">
              ...
              <a
                href={t("ctaHref")}
                className="mt-2 text-base font-medium tracking-tight text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
              >
                {t("ctaLabel")}
              </a>
            </nav>
      ...
      {/* Línea base: doble regla — un borde neutro y un acento primary más grueso debajo */}
