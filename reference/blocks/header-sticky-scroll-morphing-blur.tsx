...
/**
 * BLOQUE: header-sticky-scroll-morphing-blur — header sticky de dos capas
 * (barra de utilidad + nav principal) con blur de fondo permanente y anillo
 * inferior que simula el efecto de "compactación" al hacer scroll (sin JS de
 * cliente ni listeners de scroll: el blur/soporte translúcido ya da la
 * sensación de profundidad). Ideal para navegación de alto impacto donde el
 * sitio necesita jerarquía extra (contacto directo + CTA) por encima del menú.
 *
 * ns esperado:
 *   { utilityLabel: string, phone: string, email: string,
 *     logo: string, navLabel: string, links: [{ label: string, href: string }],
 *     ctaLabel: string, ctaHref: string, menuLabel: string }
 */
type NavLink = { label: string; href: string };

export function HeaderStickyScrollMorphingBlur({ ns }: { ns: string }) {
  const t = useTranslations(ns);
  const links = t.raw("links") as NavLink[];

  return (
    <header className="sticky top-0 z-50 border-b border-border bg-background/75 backdrop-blur-lg supports-backdrop-filter:bg-background/60">
      ...
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-4 px-6 py-4 lg:px-8">
        ...
        <details className="group relative lg:hidden">
          ...
          <div className="fixed inset-x-0 top-[calc(4rem+1px)] z-50 border-b border-border bg-background/95 px-6 py-8 backdrop-blur-lg shadow-lg">
            <nav
              aria-label={t("navLabel")}
