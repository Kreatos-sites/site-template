...
/**
 * Bloque compuesto tipo "landing de preventa/lista de espera": hero centrado
 * con captura de correo inline, cifras destacadas, grid de características,
 * timeline de proceso, testimonio destacado y banda final de registro a
 * lista de espera, en un solo componente. Úsalo para presentar un producto o
 * plataforma en fase de preventa/acceso anticipado de principio a fin.
 */
export function HeroEmailWaitlistStatsFlow({ ns }: { ns: string }) {
  ...
  return (
    <>
      ...
      <section className="border-t border-border bg-background py-(--section-gap)">
        <div className="mx-auto w-full max-w-3xl px-6 text-center lg:px-8">
          ...
          <Reveal delay={240}>
            ...
            >
              <label htmlFor="hero-email-waitlist-input" className="sr-only">
                {t("inputLabel")}
              </label>
              <input
                id="hero-email-waitlist-input"
                type="email"
                name="email"
                required
        ...
        <div className="mx-auto w-full max-w-6xl px-6 lg:px-8">
          ...
          <ol className="relative mt-16 grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-4">
            ...
            {steps.map((step, index) => {
              const Icon = ICONS[step.icon] ?? RiTimeLine;
              return (
      ...
      {/* Banda de registro a lista de espera */}
      <section className="border-t border-border bg-secondary py-(--section-gap) text-secondary-foreground">
        <div className="mx-auto w-full max-w-3xl px-6 text-center lg:px-8">
          ...
          <Reveal delay={120}>
            ...
            >
              <label
                htmlFor="hero-email-waitlist-band-input"
                className="sr-only"
              >
                {t("waitlistInputLabel")}
              </label>
              <input
                id="hero-email-waitlist-band-input"
                type="email"
                name="email"
                required
                placeholder={t("waitlistInputPlaceholder")}
                className="w-full rounded-full border border-border bg-background px-5 py-3.5 text-sm text-foreground placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
              />
              <button
                type="submit"
                className="group inline-flex shrink-0 items-center justify-center gap-2 rounded-full bg-primary px-7 py-3.5 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background focus-visible:outline-none"
