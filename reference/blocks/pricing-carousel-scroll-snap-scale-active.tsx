...
export function PricingCarouselScrollSnapScaleActive({ ns }: { ns: string }) {
  ...
  return (
    <section className="border-t border-border bg-background py-(--section-gap)">
      <div className="mx-auto w-full max-w-6xl px-6 lg:px-8">
        ...
        <div
          className={cn(
            "mt-14 flex snap-x snap-mandatory items-center gap-6 overflow-x-auto pb-8",
          ...
          {plans.map((plan, index) => (
            <Reveal key={index} delay={index * 60} className="shrink-0 snap-center">
              <article
                tabIndex={0}
                className={cn(
                  "group flex shrink-0 flex-col gap-6 rounded-lg border border-border bg-card p-8",
                  "origin-center transition-[transform,opacity,box-shadow] duration-300 ease-out will-change-transform",
                  "hover:z-10 hover:scale-105 focus-visible:z-10 focus-visible:scale-105 focus-visible:outline-none",
                  plan.active
                    ? "z-10 w-[21rem] scale-110 border-primary shadow-xl"
                    : "w-[17rem] scale-90 opacity-70 shadow-none",
                )}
              >
                <div>
